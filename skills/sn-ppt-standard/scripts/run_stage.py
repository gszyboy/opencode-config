...
def cmd_gen_image(deck: Path, page_no: int, slot_id: str) -> int:
    ...
    _GEN_RETRIES = 3
    _GEN_RETRY_DELAYS = [3, 6, 12]  # seconds

    def _try_generate() -> tuple[int, dict]:
        """Try one generation (including VLM QC). Returns (code, payload)."""
        try:
            proc = subprocess.run(cmd, capture_output=True, text=True, timeout=600)
        except subprocess.TimeoutExpired:
            return 1, {"error": "sn_agent_runner sn-image-generate timed out after 600s"}
        except FileNotFoundError as e:
            return 1, {"error": f"failed to spawn python for sn-image-base: {e}"}
        ...
        if not save_path.exists() or save_path.stat().st_size == 0:
            return 1, {"error": f"sn-image-generate returned ok but {save_path} is missing/empty"}

        # VLM quality gate (part of generation — reject = retry)
        ...
        if qc_reject is not None:
            try:
                ...
                pass
            return 1, {"error": f"vlm_qc_rejected: {qc_reject[:120]}"}

        return 0, {}

    # Retry loop (original attempt + retries)
    last_error = ""
    for attempt in range(_GEN_RETRIES + 1):
        code, payload = _try_generate()
        if code == 0:
            break
        last_error = payload.get("error", "unknown")
        if attempt < _GEN_RETRIES:
            delay = _GEN_RETRY_DELAYS[attempt]
            _progress(f"[retry] p{page_no:03d}/{slot_id} attempt {attempt+1}/{_GEN_RETRIES} failed ({last_error}), retrying in {delay}s...")
            import time
            time.sleep(delay)
    else:
        _update_asset_plan_slot(
            plan_path, page_no, slot_id,
            {"status": "failed", "quality_review": {"error": last_error[:300]}},
        )
        return _fail(f"gen-image p{page_no} {slot_id}: {last_error} (after {_GEN_RETRIES} retries)",
...
def cmd_export(deck: Path) -> int:
    ...
    # 自动收集字体（不阻断导出）
    font_rc = cmd_collect_fonts(deck)
    if font_rc != 0:
        _progress(f"[export] collect-fonts returned code {font_rc}; continuing with export anyway")
...
_SYSTEM_FONTS = {
    # Apple / macOS
    "PingFang SC", "PingFang TC", "PingFang HK", "STHeiti", "STSong",
    # Windows
    "Microsoft YaHei", "Microsoft JhengHei", "SimHei", "SimSun", "KaiTi",
    "Microsoft YaHei UI", "Microsoft YaHei Light", "Microsoft YaHei Bold",
    # Linux
    "WenQuanYi Micro Hei", "WenQuanYi Zen Hei", "WenQuanYi Micro Hei Mono",
    "WenQuanYi Zen Hei Mono", "Noto Sans CJK SC", "Noto Sans SC",
    "Source Han Sans SC", "Source Han Sans", "Source Han Serif SC",
    "AR PL UKai CN", "AR PL UMing CN", "Droid Sans Fallback",
    # Google Fonts (available but system fonts)
    "Noto Sans SC", "Noto Serif SC", "Noto Sans TC", "Noto Serif TC",
}

_FONT_CACHE_DIR = Path(os.environ.get(
    "FONT_CACHE_DIR",
    str(Path.home() / ".config" / "opencode" / "fonts"),
))


def _extract_font_names(html_dir: Path) -> set[str]:
    """Extract Google Fonts family names from all page_*.html files."""
    fonts: set[str] = set()
    for html_file in sorted(html_dir.glob("page_*.html")):
        try:
            ...
            continue
        for match in re.finditer(r'fonts\.googleapis\.com/css2\?([^"&]+)', text):
            params = match.group(1)
            for part in params.split('&'):
                if part.startswith("family="):
                    family = unquote_plus(part[7:])
...
def _download_font_ttf(font_name: str, cache_dir: Path) -> Path | None:
    """Download a Google Font (WOFF2) and convert to TTF via fontTools."""
    safe_name = font_name.replace(" ", "_")
    ttf_path = cache_dir / f"{safe_name}.ttf"
    woff2_path = cache_dir / f"{safe_name}.woff2"
    ...
    url = (
        ...
        f"?family={quote(font_name)}:wght@400;500;600;700&display=swap"
    )
    try:
        resp = httpx.get(url, timeout=15)
        css = resp.text
    except Exception as e:
        _progress(f"[font] {font_name}: fetch CSS failed - {e}")
        return None

    # 2. Parse WOFF2 URL from CSS
    m = re.search(r'url\(([^)]+)\)', css)
    if not m:
        _progress(f"[font] {font_name}: no font URL found in CSS")
        return None
    font_url = m.group(1).strip('"\'')

    # 3. Download WOFF2
    try:
        resp = httpx.get(font_url, timeout=30)
        woff2_path.write_bytes(resp.content)
    except Exception as e:
        _progress(f"[font] {font_name}: download failed - {e}")
        return None

    # 4. Convert WOFF2 → TTF via fontTools
    try:
        from fontTools.ttLib import TTFont
        font = TTFont(str(woff2_path))
        font.save(str(ttf_path))
        woff2_path.unlink(missing_ok=True)
        return ttf_path
    except Exception as e:
        _progress(f"[font] {font_name}: fontTools convert failed - {e}")

    # 5. Fallback: woff2 npm tool
    try:
        result = subprocess.run(
            ["npx", "woff2ttf", str(woff2_path), str(ttf_path)],
    ...
    try:
        result = subprocess.run(
            ["npx", "woff2", "--ttf", str(woff2_path), str(ttf_path)],
            capture_output=True, text=True, timeout=30,
        )
        if result.returncode == 0:
            woff2_path.unlink(missing_ok=True)
    ...
    _progress(f"[font] {font_name}: all conversion methods failed")
    woff2_path.unlink(missing_ok=True)
    return None


def cmd_collect_fonts(deck: Path) -> int:
    """Collect fonts used in generated HTML pages.

    - Scans page_*.html for Google Fonts references
    - Skips system fonts (cannot redistribute)
    - Downloads missing fonts → converts WOFF2 to TTF via fontTools
    - Copies to <deck_dir>/fonts/ and ~/.config/opencode/fonts/
    """
    pages_dir = deck / "pages"
    if not pages_dir.exists():
        return _fail("pages/ missing")
    ...
    font_names = _extract_font_names(pages_dir)
    collected: list[dict] = []

    for font_name in sorted(font_names):
        # Skip system fonts
        if font_name in _SYSTEM_FONTS:
            collected.append({
                "font_name": font_name,
                "status": "skipped",
                "note": "system font, cannot be redistributed",
            })
            continue

        safe_name = font_name.replace(" ", "_")
        cache_path = _FONT_CACHE_DIR / f"{safe_name}.ttf"
        ...
        # Download & convert
        ttf_path = _download_font_ttf(font_name, _FONT_CACHE_DIR)
        if ttf_path:
            shutil.copy2(ttf_path, target_path)
            collected.append({
                "font_name": font_name,
                "status": "downloaded",
                "path": str(target_path),
            ...
            collected.append({
                "font_name": font_name,
                "status": "failed",
                "note": "Could not download or convert",
            })

    ok_count = sum(1 for c in collected if c["status"] in ("ok", "downloaded", "cached"))
    fail_count = sum(1 for c in collected if c["status"] == "failed")
    skip_count = sum(1 for c in collected if c["status"] == "skipped")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    for name in ("preflight", "style", "outline", "asset-plan", "export", "collect-fonts"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.cmd == "collect-fonts":
        return cmd_collect_fonts(deck)
