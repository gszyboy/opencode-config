...
- `typography` must match the design_style's personality. **The downstream pipeline supports Google Fonts auto-download**, so feel free to use expressive Google Fonts — ZCOOL KuaiLe for playful/rounded, Ma Shan Zheng for traditional/brush-script, Noto Serif SC for formal, etc. Avoid generic defaults like "Arial" / "SimHei".
