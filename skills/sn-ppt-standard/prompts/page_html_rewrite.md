...
**默认语言是中文。`language` 为 `zh`（绝大多数情况）时，整段 query 必须全部使用中文，不得出现英文词汇或中英混杂。** 只有当 `language` 明确为 `en` 时，才使用英文。

- 下游生成的 HTML 文字内容会沿用 query 的语言 —— 所以 query 必须纯中文，最终 PPT 页面才会是纯中文。用户 query 是中文却生成出英文页面，属于**严重回归**。
- 如果 outline 里出现了混杂语言（例如 title 是英文、bullets 是中文），**以 `language` 字段为准统一表达**，把英文翻译成中文。
...
请在 query 中引导 HTML 生成器根据页面风格和主题选择合适的 Google Fonts 字体，不要只依赖系统默认字体。可以建议："标题使用活泼的字体（如 ZCOOL KuaiLe）、正文用清晰的无衬线字体"——具体字体名由 HTML 生成器根据风格自行决定。
