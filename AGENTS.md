...
- 技术术语、工具名、代码关键字、API 名称、标准缩写（如 HTTP、JSON、SQL、MCP、ORM）可保留英文不必强行翻译，但要进行中文注释说明其含义。
...
## 行为准则
**如果用户的请求会导致糟糕的结果，你应该礼貌地指出并提供更好的替代方案**
### 1. Think Before Coding
**不假设、不隐藏困惑、不绕过问题。**

- 有疑问必须先问清楚，不能猜着做
- 有多种解释时，先呈现再选择，不能擅自决定
- 发现更简单的方案要说出来，该反驳就反驳
- 遇到不清楚的，直接停下来问
...
- 不为单次使用创建抽象
- 不加没要求的"灵活性"
- 不处理不可能发生的错误场景（"不可能发生"指业务逻辑上绝对无入口的场景，如单机项目防分布式死锁；输入校验、边界条件等必须按 Goal-Driven Execution 写测试）
- **优先**清晰可读，而非强行压缩行数
- 复杂逻辑应拆分为有意义的函数
...
**风格例外**（改动范围内允许）：
- 安全规范优先级高于风格一致性，发现安全隐患时无需遵循"匹配现有风格"原则
...
```
## 需求澄清流程（禁止直接写代码）
> 豁免：仅修改样式/文案/常量值且不影响交互逻辑的原子级需求，复述确认后可跳过完整流程，直接进入编码。
- 收到任何需求后，必须先执行以下步骤，**禁止直接生成代码**：
1. 复述需求，确认理解无误
2. 识别模糊点（数据范围、边界条件、异常场景、权限角色）
3. 列出(如有)涉及的数据库表、API 接口、前端页面
4. 输出《任务拆解清单》，用户确认后再进入编码阶段
...
**超出预算处理**：
- 若预估已超出扩展模式上限（150行），则不再"先干一部分"，直接按决策升级机制进入架构模式，先输出评估报告，等待用户授权。
1. 先完成核心改动（符合预算部分）
2. 说明剩余工作需要的预算
3. 等待用户确认后再继续
...
**第一优先：observer**
- observer agent它使用 mimo-v2.5 / gemini-3.1-pro 等 vision 模型
- 可直接分析图像内容并返回文字描述

**第二优先：MiniMax_understand_image**
- 直接调用 MiniMax 多模态模型
- 支持图片分析，返回文字描述
- 当 observer agent 不可用时使用
...
- 优先使用 **playwright-cli** 命令行工具，避免在项目中引入 playwright 测试库的代码依赖
...
- L4 增量：修复bug、加小功能（≤80行）
- L5 重构：较大改动（分阶段授权）

## 文档矛盾解决
- `AGENTS.md`/`docs/目录`等文档如有自相矛盾之处或有冲突,请明确指出'文档存在冲突或矛盾'并提示用户获取授权后解决
